...
//fin de mostrar numero de factura de la tabla comprobantes
    ...
    case 'listar':
		...
		// var_dump($estado,$idsucursal2,$_SESSION['idsucursal']);
			
		$rspta=$venta->listar($fecha_inicio,$fecha_fin,$estado,$idsucursal);
		...
		case 'selectSucursal3':
			...
			if($_SESSION['idsucursal'] == 0){
				...
				echo '<option value="Todos">Todas las Sucursales</option>';
		...
		case 'listarArticulos2':
			...
			while ($reg=$rspta->fetch_object()) {
				$data[]=array(
				"0"=>'<button class="btn btn-success" onclick="agregarDetalle('.$reg->idproducto.',\''.$reg->nombre.'\',1,0,\''.$reg->precio_venta.'\',\''.$reg->precioB.'\',\''.$reg->precioC.'\',\''.$reg->precioD.'\',\'1\',\''.$reg->proigv.'\',\''.$reg->unidadmedida.'\')"><span class="fa fa-shopping-cart"></span></button>',
				"1"=>$reg->nombre,
