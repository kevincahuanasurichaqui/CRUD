...
switch ($_GET["op"]){
	...
	case 'guardaryeditar':
		...
		if (empty($idusuario)){
			$rspta=$usuario->insertar($idpersonal,$login,$clavehash,$idsucursal,$_POST['permiso']);
			echo $rspta ? "Usuario registrado" : "No se pudieron registrar todos los datos del usuario";
		}
		else {
			$rspta=$usuario->editar($idusuario,$idpersonal,$login,$clavehash,$idsucursal,$_POST['permiso']);
					...
					if($reg->nombre == NULL){
		 			...
		 			$data[]=array(
						...
						"2"=>$nombre,
		 				...
		 				"4"=>($reg->condicion)?'<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->idusuario.')"><i class="fa fa-pencil"></i></button>'.
